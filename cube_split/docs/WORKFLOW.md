...
python3.11 -m pytest -m e2e cube_split/tests/test_partition_e2e_smoke.py
```
...
```bash
PYTHONPATH=../cube_encoder:. python3.11 -m cube_split.ingest.ray_ingest_job \
...
```bash
PYTHONPATH=../cube_encoder:. python3.11 -m cube_split.ingest.product_ingest_job \
...
```bash
PYTHONPATH=../cube_encoder:. python3.11 -m cube_split.ingest.carbon_ingest_job \
...
```bash
PYTHONPATH=../cube_encoder:. python3.11 -m cube_split.quality.optical_quality \
  --run-dir data/ray_output/logical_partition/run_YYYYMMDD_HHMMSS \
  --target-crs EPSG:4326
```
...
```bash
PYTHONPATH=../cube_encoder:. python3.11 -m cube_split.quality.product_quality \
  --run-dir data/ray_output/product/run_YYYYMMDD_HHMMSS \
  --target-crs EPSG:4326
```
...
```bash
PYTHONPATH=../cube_encoder:. python3.11 -m cube_split.read.carbon_query \
...
本文是当前操作手册。旧版专题文档、一次性联调记录和旧运行基线说明已经清理；
如需追溯历史，使用 Git 历史查看旧文档。
