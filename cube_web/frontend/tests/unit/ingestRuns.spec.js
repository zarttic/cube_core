...
describe('ingest run detail drawer', () => {
  it('treats partial failure as retryable but not cancellable and emits explicit failed scene ids', async () => {
      ...
      props: {
        ...
        detail: {
          ...
          scenes: [
            ...
            { scene_id: 'scene-failed-a', status: 'failed' },
            { scene_id: 'scene-failed-b', status: 'failed' },
          ],
        },
      },
      global: {
    ...
    expect(wrapper.text()).toContain('重试全部失败景');
    expect(wrapper.text()).not.toContain('取消运行');
    await wrapper.findAll('button').find((button) => button.text() === '重试全部失败景').trigger('click');
    expect(wrapper.emitted('retry-scenes')).toEqual([[['scene-failed-a', 'scene-failed-b']]]);
  });
});
...
describe('ingest runs store', () => {
  ...
  it('retries only the explicit failed scene ids and refreshes detail and list', async () => {
    const store = useIngestRunsStore();
    ...
    const action = store.retryFailedScenes(['scene-failed']);
    ...
    expect(requestPost).toHaveBeenCalledWith('/v1/ingest-runs/ingest-a/retry', { scene_ids: ['scene-failed'] });
  });
