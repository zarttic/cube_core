...
test('M5 keeps the literal M4 fixture handoff', () => {
  expect(datasetListFixture.items.map((item) => item.dataset_id)).toEqual([M4_HANDOFF.datasetA, M4_HANDOFF.datasetB]);
  expect(datasetDetailFixtureA.dataset_title).toBe(M4_HANDOFF.datasetATitle);
  expect(datasetDetailFixtureB.dataset_title).toBe(M4_HANDOFF.datasetBTitle);
  ...
  expect(qualityRecordsFixture.items.map((item) => item.quality_run_id)).toEqual([M4_HANDOFF.qualityRunA, M4_HANDOFF.qualityRunB]);
  expect(qualityDetailFixtureA.quality_run_id).toBe(M4_HANDOFF.qualityRunA);
  expect(qualityDetailFixtureB.quality_run_id).toBe(M4_HANDOFF.qualityRunB);
  ...
  expect(M4_HANDOFF).toMatchObject({
    datasetRouteA: '**/v1/partition/datasets/dataset-a',
    datasetRouteB: '**/v1/partition/datasets/dataset-b',
    qualityRouteA: '**/v1/quality/records/quality-run-a',
    qualityRouteB: '**/v1/quality/records/quality-run-b',
...
test('M5 discards the delayed quality detail response', async ({ page }) => {
  ...
  await expect(page.getByTestId('quality-detail-drawer')).toContainText(M4_HANDOFF.qualityRunB);
  await releaseQualityRunA();
  await expect(page.getByTestId('quality-detail-drawer')).toContainText(M4_HANDOFF.qualityRunB);
  await expect(page.getByTestId('quality-detail-drawer')).not.toContainText(M4_HANDOFF.qualityRunA);
});

test('M5 checks frozen grid options, drawer reset, and complete quality exports', async ({ page }) => {
  test.setTimeout(30_000);
  const exports = [];
  await installApiRoutes(page);
  page.on('request', (request) => {
    if (request.url().includes(M4_HANDOFF.exportBase)) exports.push(request);
  });
  ...
  await expect(page.getByTestId('dataset-detail-drawer')).toContainText(M4_HANDOFF.datasetBTitle);
  await expect(page.getByTestId('dataset-detail-drawer')).not.toContainText(M4_HANDOFF.datasetATitle);
  await page.getByTestId('dataset-detail-close').click();
