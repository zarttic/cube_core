...
const datasetFields = new Set(['dataset_id', 'dataset_code', 'dataset_title', 'data_type', 'product_type', 'assets', 'bands', 'attributes']);
const assetFields = new Set(['source_asset_id', 'cog_uri', 'checksum', 'bbox', 'crs', 'time_start', 'time_end', 'attributes']);
...
export const usePartitionStore = defineStore('partition', () => {
  ...
  function buildRequest() {
    ...
    if (dataTypes.length !== 1 || !dataTypes[0]) throw invalidRequest('同一次剖分只能提交一种数据类型。');
  ...
  async function submit() {
    const { dataType, body } = buildRequest();
    const request = submitScope.begin();
    error.value = '';
    loading.submit = true;
    try {
      const response = await requestPost(`/v1/partition/${dataType}/tasks/run`, body, { signal: request.signal });
