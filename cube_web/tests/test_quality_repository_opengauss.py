...
def open_gauss_tx():
    ...
    if not dsn:
        pytest.fail("OpenGauss quality repository tests require CUBE_WEB_POSTGRES_DSN")
...
def seeded_outputs(open_gauss_tx):
    token = uuid4().hex
    batch_id = f"quality-batch-{token}"
    task_id = f"quality-task-{token}"
    dataset_id = f"quality-dataset-{token}"
    dataset_code = f"QUALITY-{token}"
    current_version = f"quality-current-{token}"
    old_version = f"quality-old-{token}"
...
def _start(connection, run_id: UUID, *, worker_id: str = "quality-real-worker") -> QualityLease:
    connection.execute(
...
def test_quality_errors_resolve_dataset_scene_and_band_labels(open_gauss_tx, seeded_outputs: dict[str, str]) -> None:
    scene_id = f"quality-scene-{uuid4().hex}"
    asset_id = f"quality-asset-{uuid4().hex}"
    with open_gauss_tx() as connection:
        connection.execute(
            "INSERT INTO datasets (dataset_id, dataset_code, dataset_title, data_type) VALUES (%s, %s, %s, 'optical')",
            (seeded_outputs["dataset_id"], seeded_outputs["dataset_code"], seeded_outputs["dataset_code"]),
        )
        connection.execute(
            "INSERT INTO scenes (scene_id, dataset_id, scene_key, identity_key, source_uri) VALUES (%s, %s, %s, %s, %s)",
            (scene_id, seeded_outputs["dataset_id"], "LC08_TEST_SCENE", uuid4().hex, "s3://cube/source/test.tif"),
        )
        connection.execute(
            "INSERT INTO scene_assets (scene_id, asset_id, source_uri) VALUES (%s, %s, %s)",
            (scene_id, asset_id, "s3://cube/source/test.tif"),
        )
        connection.execute(
            "INSERT INTO scene_bands (scene_id, asset_id, band_code, band_name, display_order) VALUES (%s, %s, 'bqa', '质量评估', 0)",
            (scene_id, asset_id),
        )
        run = _allocate(
            connection,
        ...
        write_quality_error_batch(
            ...
            errors=(
                NewQualityError(
                    quality_error_id=uuid4(),
                    quality_run_id=run.quality_run_id,
                    rule_code="optical_band_contract",
                    error_code="invalid_band_type",
                    message="band type is invalid",
                    source_asset_id=asset_id,
                    band_code="bqa",
                ),
            ),
        )
        error = list_quality_errors(
            connection,
            quality_run_id=run.quality_run_id,
        ...
        assert (error.scene_id, error.scene_name, error.band_code, error.band_name) == (
            scene_id,
            "LC08_TEST_SCENE",
            "bqa",
            "质量评估",
        )
        connection.execute("DELETE FROM scene_bands WHERE scene_id = %s", (scene_id,))
        connection.execute("DELETE FROM scene_assets WHERE scene_id = %s", (scene_id,))
        connection.execute("DELETE FROM scenes WHERE scene_id = %s", (scene_id,))
        connection.execute("DELETE FROM datasets WHERE dataset_id = %s", (seeded_outputs["dataset_id"],))
        connection.commit()
...
def test_publication_is_active_and_withdraws_in_database(open_gauss_tx, seeded_outputs: dict[str, str]) -> None:
    ...
    try:
        ...
        with pytest.raises(PublicationPolicyRejected, match="completed ingest"):
            publish_dataset(seeded_outputs["dataset_id"], PublishRequest(), Actor("publisher", "admin"))
        with open_gauss_tx() as connection:
            connection.execute(
                "INSERT INTO datasets (dataset_id, dataset_code, dataset_title, data_type) VALUES (%s, %s, %s, 'optical')",
                (seeded_outputs["dataset_id"], seeded_outputs["dataset_code"], seeded_outputs["dataset_code"]),
            ...
            connection.execute(
                "INSERT INTO scenes (scene_id, dataset_id, scene_key, identity_key, source_uri) VALUES (%s, %s, %s, %s, %s)",
                (scene_id, seeded_outputs["dataset_id"], scene_id, scene_id, "s3://cube/test/source.tif"),
            )
            connection.execute(
            ...
            connection.execute(
                "INSERT INTO ingest_runs (ingest_run_id, partition_run_id, dataset_id, status, completed_at) "
                "VALUES (%s, %s, %s, 'completed', now())",
                (ingest_run_id, partition_run_id, seeded_outputs["dataset_id"]),
            )
            connection.execute(
                "INSERT INTO ingest_run_scenes "
                "(ingest_run_id, scene_id, partition_run_id, output_version, status, idempotency_key) "
        ...
        with open_gauss_tx() as connection:
            connection.execute(
                "UPDATE partition_runs SET status = 'completed' WHERE partition_run_id = %s",
                (partition_run_id,),
            )
        ...
        with open_gauss_tx() as connection:
            ...
            connection.execute("DELETE FROM ingest_runs WHERE ingest_run_id = %s", (ingest_run_id,))
            connection.execute("DELETE FROM partition_runs WHERE partition_run_id = %s", (partition_run_id,))
            connection.execute("DELETE FROM scenes WHERE scene_id = %s", (scene_id,))
            connection.execute("DELETE FROM datasets WHERE dataset_id = %s", (seeded_outputs["dataset_id"],))
        set_partition_domain_store(previous_store)
