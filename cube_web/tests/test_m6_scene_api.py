...
from cube_web.services.m6_scene_repository import (
    OpenGaussSceneRepository,
    _load_schema_datasets,
    _partition_scene_idempotency_key,
)
from cube_web.services.m6_scene_service import SceneDomainService, adapt_scene_partition_request
...
class _Repository:
    ...
    def materialize_partition_datasets(self, _request):
...
def api():
    ...
    app.include_router(create_m6_scene_router(SceneDomainService(repository, workflow)), prefix="/v1")
...
def test_partition_projection_advances_m6_dataset_current_output() -> None:
    statements = []
...
def test_legacy_dual_write_is_disabled_for_in_memory_runtime() -> None:
    service = SceneDomainService(_Repository(), _Workflow())

    assert service.should_sync_import(InMemoryPartitionJobStore()) is False
    assert service.should_sync_import(object()) is True
...
def test_legacy_adapter_does_not_copy_source_batch_ids_into_dataset_identity() -> None:
    ...
    adapted = adapt_scene_partition_request(request, datasets)
...
def test_legacy_scene_without_dataset_uses_batch_independent_provisional_group() -> None:
    manifest = {
        "data_type": "optical",
    ...
    first = _load_schema_datasets(manifest, load_batch_id="load-001")[0]
    later = _load_schema_datasets(manifest, load_batch_id="load-002")[0]

    assert first["dataset_id"] == later["dataset_id"]
    assert first["status"] == "draft"
    assert first["assignment_confidence"] == 0
    assert first["auto_ingest_allowed"] is False
    assert "load-001" not in first["dataset_id"]
    assert first["scenes"][0]["scene_key"] == later["scenes"][0]["scene_key"]


def test_legacy_flat_assets_with_generic_scene_id_keep_distinct_scene_identity() -> None:
    manifest = {
        "data_type": "optical",
        "source_system": "loader-a",
        "assets": [
            {
                "scene_id": "Optical_Scene_Grid",
                "asset_id": "asset-scene-001-band-1",
                "source_uri": "s3://cube/source/scene-001.tif",
                "sensor": "HiGLASS",
                "bands": ["sr_band1"],
            },
            {
                "scene_id": "Optical_Scene_Grid",
                "asset_id": "asset-scene-002-band-1",
                "source_uri": "s3://cube/source/scene-002.tif",
                "sensor": "HiGLASS",
                "bands": ["sr_band1"],
            },
        ],
    ...
    dataset = _load_schema_datasets(manifest, load_batch_id="load-001")[0]

    assert [scene["scene_key"] for scene in dataset["scenes"]] == [
        "asset-scene-001-band-1",
        "asset-scene-002-band-1",
    ]
    assert len({scene["identity_key"] for scene in dataset["scenes"]}) == 2
...
def test_multi_dataset_schema_import_does_not_flatten_into_legacy_batch() -> None:
    class Workflow:
        store = object()
        called = False

        def import_schema(self, _payload):
            self.called = True
            raise AssertionError("M6 manifest must not enter the single-data-type legacy importer")
    ...
    result = import_partition_schema_payload(workflow, SceneService(), payload)

    assert workflow.called is False
    assert result["status"] == "imported"
    assert result["load_batch_id"] == "load-mixed"
    assert result["scene_domain"]["dataset_count"] == 2


def test_legacy_dual_write_reports_reconciliation_when_scene_projection_fails() -> None:
    class Workflow:
        store = object()

        def import_schema(self, payload):
            return {"batch_id": payload["batch_id"], "status": "pending"}

    class SceneService:
        def should_sync_import(self, _store):
            return True

        def import_load_schema(self, _payload):
            raise RuntimeError("database unavailable")

    result = import_partition_schema_payload(
        Workflow(),
        SceneService(),
        {"batch_id": "legacy-load", "data_type": "optical", "assets": [{"source_uri": "s3://cube/a.tif"}]},
    )

    assert result["batch_id"] == "legacy-load"
    assert result["scene_domain"] == {
        "status": "sync_failed",
...
def test_public_import_contract_accepts_explicit_load_batch_id() -> None:
    ...
    assert payload.batch_id == "load-mixed"
