...
from cube_web.services.m6_dataset_management import (
    ...
    repository = InMemoryDatasetManagementRepository(
        datasets={
            "dataset-a": {
                ...
                "legacy_partition_dataset_id": "partition-dataset-a",
            ...
            "dataset-b": {
                ...
                "legacy_partition_dataset_id": None,
        ...
        details={
            "dataset-a": {
                ...
                "ingest-records": [{"ingest_run_id": "ingest-a", "scene_id": "scene-a2", "status": "failed", "error_message": "bad tile"}],
...
def test_failed_scene_retry_and_actions_use_injected_domain_hooks() -> None:
    client, repository, hooks = _fixture()
    retry = client.post("/v1/datasets/dataset-a/scenes/scene-a2/ingest-retry", json={})
    assert retry.status_code == 202
    assert retry.json()["status"] == "queued"
    conflict = client.post("/v1/datasets/dataset-a/scenes/scene-a2/ingest-retry", json={})
    ...
    repository.details["dataset-a"]["ingest-records"].append({
        "ingest_run_id": "ingest-a-completed", "scene_id": "scene-a2", "status": "completed"
    })
    ...
    assert hooks == {
        "quality": [("partition-dataset-a", "admin")],
        "publish": [("partition-dataset-a", "admin")],
        "withdraw": [("partition-dataset-a", publication_id, "数据管理页面撤回", "admin")],
    }
...
def test_publish_requires_passed_quality_even_after_ingest_completed() -> None:
    client, repository, hooks = _fixture()
    repository.details["dataset-a"]["ingest-records"].append({
        "ingest_run_id": "ingest-completed", "scene_id": "scene-a1", "status": "completed"
    })
...
def test_dataset_mutations_require_admin() -> None:
    ...
    requests = (
        ...
        client.post("/v1/datasets/dataset-a/scenes/scene-a2/ingest-retry", json={}, headers=headers),
...
def test_opengauss_provenance_uses_validated_lineage_not_run_level_fact_joins() -> None:
    sql, params = OpenGaussDatasetManagementRepository._detail_sql("provenance", "dataset-a")
    assert "migration_lineage" in sql
    assert "'legacy:' || ml.source_table" in sql
    assert "JOIN rs_raw_scene_asset r ON r.run_id" not in sql
    assert "current_scene.dataset_id" in sql
    assert len(params) == 8
    ...
    assert "'partition-run-legacy-' || md5(o.task_id)" in output_sql
    assert "ml.provenance->>'tile_uri'" in tile_sql
    assert "ml.provenance->>'space_code'" in tile_sql
    assert "ml.provenance->>'value_ref_uri'" in index_sql
    assert "current_scene.dataset_id" in tile_sql and "current_scene.dataset_id" in index_sql
