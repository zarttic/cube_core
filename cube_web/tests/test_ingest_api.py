...
def test_retry_requires_explicit_failed_band_ids() -> None:
    ...
    response = client.post(f"/v1/ingest-runs/{failed_id}/retry", json={"band_unit_ids": ["band-failed-scene"]})
    assert response.status_code == 200
    assert response.json()["scenes"][0]["status"] == "queued"
    assert client.post(f"/v1/ingest-runs/{failed_id}/retry", json={"band_unit_ids": ["band-failed-scene"]}).status_code == 409
...
def test_ingest_run_mutations_require_admin() -> None:
    ...
    retry = client.post(
        ...
        json={"band_unit_ids": ["band-failed-scene"]},
        headers=headers,
    )
    cancel = client.post(
