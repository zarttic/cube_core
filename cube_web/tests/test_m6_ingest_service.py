...
from cube_web.services.m6_ingest_repository import (
    IngestConflict,
...
def test_legacy_failed_scene_without_error_can_be_retried_and_read() -> None:
...
def test_open_gauss_model_accepts_legacy_null_output_and_provenance() -> None:
    ...
    run = {
        ...
        "requested_by": "system",
        "error_message": None,
        "attributes": {"legacy_ingest_job_id": "job-a"},
        "created_at": now,
        "started_at": None,
        "completed_at": now,
    }
    scenes = [
        {
            ...
            "attempt_count": 0,
            "error_message": None,
            "provenance": {"source_load_batch_id": "legacy-batch"},
            "created_at": now,
            "updated_at": now,
        }
    ]
    model = OpenGaussIngestRepository._model(run, scenes)
    assert model.scenes[0].quality_run_id is None
    assert model.scenes[0].output_version is None
    assert model.scenes[0].source_load_batch_ids == ("legacy-batch",)
