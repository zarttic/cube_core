...
class OpenGaussSceneRepository:
    ...
    def upsert_load_schema(self, payload: dict[str, Any]) -> dict[str, Any]:
        legacy_batch_id = str(payload.get("batch_id") or "").strip()
        explicit_load_batch_id = str(payload.get("load_batch_id") or "").strip()
        if legacy_batch_id and explicit_load_batch_id and legacy_batch_id != explicit_load_batch_id:
            raise ValueError("batch_id and load_batch_id must match when both are provided")
        load_batch_id = str(payload.get("load_batch_id") or payload.get("batch_id") or "").strip()
            ...
            with connection.cursor() as cursor:
                ...
                if outcomes:
                    for dataset_id, outcome in outcomes.items():
                        ...
                        if str(outcome.get("status") or "") in {"completed", "partial_failure"} and outcome.get("output_version"):
                            cursor.execute(
                                """
...
def _load_schema_datasets(payload: dict[str, Any], *, load_batch_id: str) -> list[dict[str, Any]]:
    ...
    if isinstance(raw_datasets, list) and raw_datasets:
        ...
        data_type = str(payload.get("data_type") or "unknown").strip().lower()
        items = payload.get("observations") if data_type == "carbon" else payload.get("assets")
        if items is None:
            normalized = payload.get("normalized_payload") or {}
            items = normalized.get("selected_observations") if data_type == "carbon" else normalized.get("selected_assets")
        grouped: dict[str, list[dict[str, Any]]] = {}
        for item in items or []:
            if not isinstance(item, dict):
                continue
            dataset_id = str(item.get("dataset_id") or payload.get("dataset_id") or "").strip()
            if not dataset_id:
                grouping = ":".join(
                    str(item.get(field) or payload.get(field) or "unknown").strip().lower()
                    for field in ("source_namespace", "sensor", "product_family", "product_type")
                )
                dataset_id = f"provisional-{sha256(f'{data_type}:{grouping}'.encode()).hexdigest()[:24]}"
            grouped.setdefault(dataset_id, []).append(item)
        datasets = [
            _normalize_load_dataset(
                {
                    "dataset_id": dataset_id,
                    "dataset_code": dataset_id,
                    "dataset_title": str(
                        payload.get("dataset_title")
                        or f"Provisional {data_type} {items[0].get('sensor') or items[0].get('product_family') or 'dataset'}"
                    ),
                    "data_type": data_type,
                    "status": "draft",
                    "assignment_confidence": 0,
                    "assignment_issue": "loader manifest did not provide dataset_id",
                    "auto_ingest_allowed": False,
                    "attributes": {
                        "provisional": True,
                        "auto_ingest_allowed": False,
                        "assignment_basis": "data_type+source_namespace+sensor+product_family+product_type",
                    },
                    "scenes": items,
                },
                payload=payload,
...
def _normalize_load_dataset(item: Any, *, payload: dict[str, Any], load_batch_id: str) -> dict[str, Any]:
    ...
    data_type = str(item.get("data_type") or payload.get("data_type") or "unknown").strip().lower()
    if data_type not in {"optical", "radar", "product", "carbon", "entity", "unknown"}:
...
def _normalize_load_scene(
    ...
    *,
    ...
    if not isinstance(raw_assets, list) or not raw_assets:
        raw_assets = [item]
    assets = [_normalize_scene_asset(asset, data_type=data_type, index=asset_index) for asset_index, asset in enumerate(raw_assets)]
...
def _normalize_scene_asset(item: Any, *, data_type: str, index: int) -> dict[str, Any]:
    if not isinstance(item, dict):
    ...
    for band_index, raw_band in enumerate(raw_bands):
        ...
        bands.append({
            ...
            "band_type": str(band.get("band_type") or ("variable" if data_type in {"product", "carbon"} else "spectral")),
            "unit": band.get("unit"),
            "display_order": int(band.get("display_order", band_index)),
            "attributes": _json_object(band.get("attributes")),
        })
    attributes = _json_object(item.get("attributes"))
    for key in (
        "resolution", "resolution_m", "spatial_resolution", "spatial_resolution_m",
