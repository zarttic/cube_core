...
from cube_web.services.m6_ingest_repository import scene_idempotency_key
...
def create_ingest_runs_after_quality(
    ...
    *,
    ...
    """Create M6 ingest intents in the caller's quality-completion transaction."""
    if quality_status not in {"pass", "warn"}:
        return 0
    _require_m6_tables(tx)
    ...
    with tx.cursor(row_factory=dict_row) as cur:
        cur.execute(
            ...
            WHERE d.dataset_id=%s
              AND EXISTS (
                ...
                WHERE pd.dataset_id=COALESCE(NULLIF(d.legacy_partition_dataset_id,''),d.dataset_id)
              ...
              AND EXISTS (
                ...
                WHERE prs.dataset_id=d.dataset_id AND prs.output_version=%s
                  AND prs.status='completed' AND pr.status IN ('completed','partial_failure')
              )
            """,
        ...
        cur.execute(
            ...
            "AND pr.status IN ('completed','partial_failure') ORDER BY prs.partition_run_id,prs.scene_id",
...
def _require_m6_tables(tx: Any) -> None:
    required = {
        "datasets",
        "load_batch_scenes",
        "partition_runs",
    ...
    if missing:
        raise RuntimeError(f"M6 auto-ingest schema is incomplete: {', '.join(sorted(missing))}")
