...
class InMemoryPartitionJobStore(PartitionJobStore):
    ...
    def mark_result_manual_required(self, task_id: str, error: str, *, error_type: str) -> None:
        ...
        self.batches[attempt["batch_id"]].update(
            status="manual_required",
            last_error=error,
    ...
    def ensure_schema(self) -> None:
        ...
        with self._schema_lock:
            ...
            with self._connect() as conn:
                ...
                # Install the versioned partition/quality domain only after
                # the scheduler tables required by its foreign keys exist.
...
def _stable_asset_key(value: str, idx: int) -> str:
    ...
    return hashlib.sha1(f"{idx}:{value}".encode("utf-8")).hexdigest()[:16]


def normalized_dataset_asset_id(batch_id: str, dataset_id: str, source_uri: str, asset_index: int) -> str:
    """Return the stable runtime asset identity used by normalized datasets."""
