...
class DatasetManagementService:
    ...
    def request_quality(self, dataset_id: str, actor: Any) -> dict[str, Any]:
        ...
        if target_id is None:
            raise DatasetManagementConflict("M6-native quality is not available for this dataset")
    ...
    def publish(self, dataset_id: str, actor: Any) -> dict[str, Any]:
        ...
        target_id = self.repository.action_dataset_id(dataset_id)
        if target_id is None:
            raise DatasetManagementConflict("publication requires an M6-native quality snapshot")
...
class InMemoryDatasetManagementRepository:
    ...
    def action_dataset_id(self, dataset_id: str) -> str | None:
        with self._lock:
            ...
            return row.get("legacy_partition_dataset_id", dataset_id)
...
class OpenGaussDatasetManagementRepository:
    ...
    def action_dataset_id(self, dataset_id: str) -> str | None:
        with self._connection() as connection, connection.cursor(row_factory=dict_row) as cursor:
            cursor.execute(
                """
                SELECT d.legacy_partition_dataset_id,
                  EXISTS (SELECT 1 FROM partition_datasets pd WHERE pd.dataset_id=d.dataset_id) AS native_output_exists
            ...
            if row is None:
                raise ManagedDatasetNotFound(dataset_id)
            legacy_id = row["legacy_partition_dataset_id"]
            if legacy_id:
                return str(legacy_id)
            return dataset_id if row["native_output_exists"] else None
    ...
    def _overview_sql(where: str) -> str:
        return f"""
            SELECT d.dataset_id,d.dataset_code,d.dataset_title,d.data_type,d.product_type,d.status,d.current_output_version,
                   ...
                   COALESCE((SELECT pd.quality_status FROM partition_datasets pd WHERE pd.dataset_id=d.legacy_partition_dataset_id),'pending') AS quality_status,
                   COALESCE((SELECT pp.status FROM partition_publications pp WHERE pp.dataset_id=d.legacy_partition_dataset_id ORDER BY pp.requested_at DESC LIMIT 1),'unpublished') AS publish_status
            FROM datasets d LEFT JOIN scenes s ON s.dataset_id=d.dataset_id
            WHERE {where or 'TRUE'}
    ...
    def _filters(query: ManagedDatasetQuery) -> tuple[str, tuple[Any, ...]]:
        ...
        if query.quality_status:
            clauses.append("COALESCE((SELECT pd.quality_status FROM partition_datasets pd WHERE pd.dataset_id=d.legacy_partition_dataset_id),'pending')=%s")
            params.append(query.quality_status)
        if query.publish_status:
            clauses.append("COALESCE((SELECT pp.status FROM partition_publications pp WHERE pp.dataset_id=d.legacy_partition_dataset_id ORDER BY pp.requested_at DESC LIMIT 1),'unpublished')=%s")
    ...
    def _detail_sql(detail: str, dataset_id: str) -> tuple[str, tuple[Any, ...]]:
        provenance = [
            ...
            "SELECT 'legacy:' || ml.source_table,ml.source_key,ml.scene_id,ml.entity_type,ml.created_at,json_build_object('entity_id',ml.entity_id,'ingest_run_id',ml.ingest_run_id,'original_dataset_id',ml.dataset_id,'confidence',ml.confidence,'assignment_issue',ml.assignment_issue,'provenance',ml.provenance::json)::jsonb FROM migration_lineage ml LEFT JOIN scenes current_scene ON current_scene.scene_id=ml.scene_id WHERE (ml.scene_id IS NOT NULL AND current_scene.dataset_id=%s) OR (ml.scene_id IS NULL AND ml.dataset_id=%s)",
        ]
        provenance_params = 8
        queries = {
            ...
            "outputs": """
                SELECT o.output_version,'partition-run-legacy-' || md5(o.task_id) AS partition_run_id,NULL::text AS scene_id,o.status,
                       ...
                       o.created_at,COALESCE(o.completed_at,o.failed_at) AS updated_at,'legacy_output' AS source_kind
                  FROM partition_output_versions o JOIN datasets d ON d.legacy_partition_dataset_id=o.dataset_id WHERE d.dataset_id=%s
            ...
            """,
            "grid": """
                SELECT g.output_id,g.output_version,g.grid_type,g.grid_level,g.grid_level_name,g.space_code,g.bbox,g.geometry,g.created_at,'legacy_grid_cell' AS source_kind
                  FROM partition_grid_cells g JOIN datasets d ON d.legacy_partition_dataset_id=g.dataset_id WHERE d.dataset_id=%s
            ...
            """,
            "tiles": """
                SELECT t.output_id,t.output_version,t.space_code,t.tile_uri,t.status,t.created_at,'legacy_tile' AS source_kind,NULL::jsonb AS provenance
                  FROM partition_tiles t JOIN datasets d ON d.legacy_partition_dataset_id=t.dataset_id WHERE d.dataset_id=%s
                UNION ALL SELECT ml.entity_id,ml.provenance->>'tile_version',ml.provenance->>'space_code',ml.provenance->>'tile_uri','materialized',ml.created_at,'migration_lineage',ml.provenance
                  FROM migration_lineage ml JOIN scenes current_scene ON current_scene.scene_id=ml.scene_id
                  WHERE current_scene.dataset_id=%s AND ml.entity_type='entity_tile'
                ORDER BY created_at DESC
            """,
            "indexes": """
                SELECT i.output_id,i.output_version,i.space_code,i.value_ref_uri,i.st_code,i.created_at,'legacy_index' AS source_kind,NULL::jsonb AS provenance
                  FROM partition_indexes i JOIN datasets d ON d.legacy_partition_dataset_id=i.dataset_id WHERE d.dataset_id=%s
                UNION ALL SELECT ml.entity_id,ml.provenance->>'cube_version',ml.provenance->>'space_code',ml.provenance->>'value_ref_uri',ml.provenance->>'st_code',ml.created_at,'migration_lineage',ml.provenance
                  FROM migration_lineage ml JOIN scenes current_scene ON current_scene.scene_id=ml.scene_id
                  WHERE current_scene.dataset_id=%s AND ml.entity_type='cube_cell'
                ORDER BY created_at DESC
            """,
            ...
            "quality": "SELECT q.* FROM partition_quality_runs q JOIN datasets d ON d.legacy_partition_dataset_id=q.dataset_id WHERE d.dataset_id=%s ORDER BY q.created_at DESC",
            "publications": "SELECT p.* FROM partition_publications p JOIN datasets d ON d.legacy_partition_dataset_id=p.dataset_id WHERE d.dataset_id=%s ORDER BY p.requested_at DESC",
            "provenance": " UNION ALL ".join(provenance) + " ORDER BY created_at DESC",
        }
        if detail not in queries:
            raise ValueError(f"unknown dataset detail: {detail}")
        params = (dataset_id,) * (provenance_params if detail == "provenance" else (2 if detail in {"outputs", "grid", "tiles", "indexes"} else 1))
