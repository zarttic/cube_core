...
def request_manual_ingest_collection(partition_run_id: str, band_unit_ids: set[str], *, requested_by: str) -> dict[str, Any]:
    """Create ingest runs for exact quality-approved band units in one partition batch."""
    ...
    with store.transaction() as tx:
        with tx.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """
                SELECT g.band_unit_id, prs.scene_id, prs.dataset_id, prs.output_version,
                       q.quality_run_id, q.status AS quality_status
                FROM partition_data_unit_grid_status g
                JOIN partition_run_scenes prs ON prs.partition_run_id=g.partition_run_id AND prs.scene_id=g.scene_id
        ...
        if len(rows) != len(band_unit_ids):
            selected = {str(row["band_unit_id"]) for row in rows}
            missing = sorted(band_unit_ids - selected)
            raise ManualIngestRejected(f"selected band units are not eligible for ingest: {', '.join(missing)}")
        grouped: dict[tuple[str, str, str, str], set[str]] = {}
        for row in rows:
            grouped.setdefault((str(row["dataset_id"]), str(row["output_version"]), str(row["quality_run_id"]), str(row["quality_status"])), set()).add(str(row["band_unit_id"]))
        created = 0
        for (dataset_id, output_version, quality_run_id, quality_status), selected in grouped.items():
            created += create_ingest_runs_after_quality(
                ...
                requested_by=requested_by, manual=True, band_unit_ids=selected,
            )
        return {"partition_run_id": partition_run_id, "selected_band_count": len(rows), "created": created}
...
def create_ingest_runs_after_quality(
    ...
    *,
    ...
    band_unit_ids: set[str] | None = None,
) -> int:
    ...
    with tx.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute(
            "SELECT prs.partition_run_id,prs.scene_id,prs.dataset_id,prs.output_version,prs.status,g.band_unit_id "
            ...
            "AND pr.status='completed' AND g.partition_status='completed' AND g.quality_status IN ('pass','warn') "
            "ORDER BY prs.partition_run_id,prs.scene_id,g.band_unit_id",
        ...
        if band_unit_ids is not None:
            completed_rows = [row for row in completed_rows if str(row["band_unit_id"]) in band_unit_ids]
        ...
        candidates = [
            PartitionSceneOutput(
                ...
                band_unit_ids=tuple(sorted(set(item["band_unit_ids"]))),
            )
...
def _insert_request(cur: Any, request: CreateIngestRun) -> int:
    scene_keys = {
        scene.scene_id: scene_idempotency_key(request.dataset_id, scene.scene_id, scene.output_version, scene.band_unit_ids)
    ...
    selected_identity = "|".join(
        f"{scene.scene_id}:{','.join(sorted(scene.band_unit_ids))}"
        for scene in sorted(pending, key=lambda item: item.scene_id)
    )
    identity = (
        f"{request.dataset_id}\0{request.partition_run_id}\0{pending[0].output_version}"
        f"\0{pending[0].quality_run_id}\0{selected_identity}"
    )
    ...
    try:
        with cur.connection.transaction():
            cur.execute(
                ...
                "(ingest_run_id,scene_id,partition_run_id,output_version,band_unit_ids,status,idempotency_key,provenance) "
                    ...
                    Jsonb(
                        {
                            ...
                            "band_unit_ids": list(scene.band_unit_ids),
                        }
                    ),
                ),
            )
