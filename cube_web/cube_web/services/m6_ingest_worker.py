...
def process_queued_ingest_scenes(
    *,
    ...
    if not hasattr(repo, "claim_queued_outputs"):
        # Test doubles and old adapters remain usable, but production always
        # claims at Dataset/output granularity.
        scenes = repo.claim_queued_scenes(limit=limit)
        claimed = tuple(
            {"dataset_id": item["dataset_id"], "output_version": item["output_version"], "items": (item,)}
            for item in scenes
        )
    else:
    ...
    for group in claimed:
        ...
        try:
            ...
            ingest_job_id = f"m6-ingest-{sha256(identity.encode()).hexdigest()[:24]}"
    ...
    with repository.pool.connection() as connection, connection.cursor(row_factory=dict_row) as cur:
        cur.execute(
            ...
            SELECT d.current_output_version,
                   COALESCE(NULLIF(d.legacy_partition_dataset_id,''),d.dataset_id) AS output_dataset_id,
