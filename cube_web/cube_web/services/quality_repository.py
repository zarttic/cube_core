...
_ERROR_SORT_COLUMNS = {
    ...
    "quality_error_id": "e.quality_error_id",
    "rule_code": "e.rule_code",
    "error_code": "e.error_code",
}

_QUALITY_ERROR_SELECT = (
    "SELECT e.quality_error_id, e.quality_run_id, e.rule_code, "
    "COALESCE(e.source_asset_id, i.source_asset_id, t.source_asset_id) AS source_asset_id, "
    "COALESCE(e.band_code, i.band_code, t.band_code) AS band_code, "
    "sb.band_name, s.scene_id, s.scene_key AS scene_name, "
    "e.tile_id, e.index_id, e.output_id, e.row_number, e.field_name, e.error_code, e.message, e.context, e.created_at "
)
_QUALITY_ERROR_FROM = (
    "FROM partition_quality_errors e "
    "LEFT JOIN partition_indexes i ON i.dataset_id = e.dataset_id AND i.output_version = e.output_version "
    "AND (i.output_id = e.index_id OR i.output_id = e.output_id) "
    "LEFT JOIN partition_tiles t ON t.dataset_id = e.dataset_id AND t.output_version = e.output_version "
    "AND (t.output_id = e.tile_id OR t.output_id = e.output_id) "
    "LEFT JOIN scenes s ON s.dataset_id = e.dataset_id AND EXISTS ("
    "SELECT 1 FROM scene_assets scene_asset_match WHERE scene_asset_match.scene_id = s.scene_id "
    "AND scene_asset_match.asset_id = COALESCE(e.source_asset_id, i.source_asset_id, t.source_asset_id)) "
    "LEFT JOIN scene_assets sa ON sa.scene_id = s.scene_id "
    "AND sa.asset_id = COALESCE(e.source_asset_id, i.source_asset_id, t.source_asset_id) "
    "LEFT JOIN scene_bands sb ON sb.scene_id = s.scene_id AND sb.asset_id = sa.asset_id "
    "AND sb.band_code = COALESCE(e.band_code, i.band_code, t.band_code) "
)
...
def _quality_run(row: dict[str, Any], *, is_current: bool) -> QualityRun:
    ...
    return QualityRun(
        ...
        grid_level=None if row.get("grid_level") is None else int(row["grid_level"]),
...
def _error_predicate(quality_run_id: UUID, filters: QualityErrorFilter, *, table_alias: str = "") -> tuple[str, list[Any]]:
    prefix = f"{table_alias}." if table_alias else ""
    clauses = [f"{prefix}quality_run_id = %s"]
    ...
    *,
    ...
    with tx.cursor() as cur:
        ...
        cur.executemany(
            ...
            "(quality_error_id, quality_run_id, dataset_id, output_version, rule_code, source_asset_id, band_code, tile_id, index_id, "
...
def list_quality_errors(
    ...
    *,
    ...
    where, params = _error_predicate(quality_run_id, filters, table_alias="e")
    direction = "DESC" if sort_order == "desc" else "ASC"
    with tx.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"{_QUALITY_ERROR_SELECT}{_QUALITY_ERROR_FROM}"
            f"WHERE {where} ORDER BY {_ERROR_SORT_COLUMNS[sort_by]} {direction}, e.quality_error_id {direction} LIMIT %s OFFSET %s",
            (*params, limit, offset),
        )
...
def count_quality_errors(tx: PartitionDomainTransaction, *, quality_run_id: UUID, filters: QualityErrorFilter) -> int:
    where, params = _error_predicate(quality_run_id, filters, table_alias="e")
    with tx.cursor(row_factory=dict_row) as cur:
        cur.execute(f"SELECT count(*) AS total FROM partition_quality_errors e WHERE {where}", params)
        row = cur.fetchone()
...
def iter_quality_errors(
    ...
    *,
    ...
    with tx.cursor(name=cursor_name, row_factory=dict_row) as cur:
        ...
        cur.execute(
            f"{_QUALITY_ERROR_SELECT}{_QUALITY_ERROR_FROM}"
            f"WHERE {where} ORDER BY e.quality_error_id",
            params,
        )
        for row in cur:
