...
from cube_web.services.m6_ingest_contracts import CreateIngestRun, IngestRun, IngestRunScene, IngestSummary
...
def scene_idempotency_key(dataset_id: str, scene_id: str, output_version: str) -> str:
    value = f"{dataset_id}\0{scene_id}\0{output_version}".encode()
...
class IngestRepository(Protocol):
    ...
    def claim_queued_scenes(self, *, limit: int = 10) -> tuple[dict[str, str], ...]: ...
...
class InMemoryIngestRepository:
    ...
    def create(self, ingest_run_id: str, request: CreateIngestRun) -> IngestRun:
        with self._lock:
            ...
            owners = {
                ...
                if (key := scene_idempotency_key(request.dataset_id, scene.scene_id, scene.output_version)) in self.idempotency
            }
            if owners:
                if len(owners) == 1:
            ...
            for scene in request.scenes:
                ...
                key = scene_idempotency_key(request.dataset_id, scene.scene_id, scene.output_version)
    ...
    def _request_identity(run: dict[str, Any]) -> tuple[Any, ...]:
        scenes = tuple(
            sorted(
                (row["scene_id"], row["output_version"], row["quality_run_id"], row["source_load_batch_ids"])
    ...
    def _input_identity(request: CreateIngestRun) -> tuple[Any, ...]:
        scenes = tuple(
            sorted((scene.scene_id, scene.output_version, scene.quality_run_id, scene.source_load_batch_ids) for scene in request.scenes)
        )
...
class OpenGaussIngestRepository:
    """Transactional repository for the additive M6 ingest tables."""
        ...
        try:
            with self.pool.connection() as connection, connection.cursor(row_factory=dict_row) as cur:
                ...
                for scene in request.scenes:
                    ...
                    cur.execute(
                        "INSERT INTO ingest_run_scenes (ingest_run_id,scene_id,partition_run_id,output_version,status,idempotency_key,provenance) VALUES (%s,%s,%s,%s,'queued',%s,%s)",
                        (
                            ingest_run_id,
                            scene.scene_id,
                            request.partition_run_id,
                            scene.output_version,
                            scene_idempotency_key(request.dataset_id, scene.scene_id, scene.output_version),
                            Jsonb(provenance),
                        ),
                    )
    ...
    def _existing_request(self, request: CreateIngestRun) -> IngestRun | None:
        ...
        expected = (
            ...
            tuple(
                sorted(
                    (scene.scene_id, scene.output_version, scene.quality_run_id, scene.source_load_batch_ids) for scene in request.scenes
                )
            ),
        )
        actual = (
            ...
            tuple(
                sorted(
                    (scene.scene_id, scene.output_version, scene.quality_run_id, scene.source_load_batch_ids) for scene in existing.scenes
                )
            ),
        )
        if actual != expected:
    ...
    def claim_queued_scenes(self, *, limit: int = 10) -> tuple[dict[str, str], ...]:
        ...
        with self.pool.connection() as connection, connection.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """
                SELECT irs.ingest_run_id,irs.scene_id,ir.dataset_id,irs.output_version
                ...
                ORDER BY ir.created_at,irs.created_at
            ...
            for row in rows:
                cur.execute(
                    "UPDATE ingest_run_scenes SET status='running',attempt_count=attempt_count+1,updated_at=now() "
                    "WHERE ingest_run_id=%s AND scene_id=%s AND status='queued'",
                    (row["ingest_run_id"], row["scene_id"]),
                )
                if cur.rowcount == 1:
                    cur.execute(
                        "UPDATE ingest_runs SET status='running',started_at=COALESCE(started_at,now()),error_message=NULL "
                        "WHERE ingest_run_id=%s AND status IN ('queued','running')",
                        (row["ingest_run_id"],),
                    )
                    claimed.append({key: str(row[key]) for key in ("ingest_run_id", "scene_id", "dataset_id", "output_version")})
        ...
        with self.pool.connection() as connection, connection.cursor(row_factory=dict_row) as cur:
            ...
            for group in groups:
                ...
                for row in rows:
                    ...
                    items.append({key: str(row[key]) for key in ("ingest_run_id", "scene_id", "dataset_id", "output_version")})
                if items:
                    claimed.append({
