...
状态：当前行为与已知 UI 偏差记录。本文不是推荐设计；后续若重构入库状态机，应以代码、测试和新的迁移说明为准。
...
- `ISEA4H` 实体剖分不会单独再走一个 confirm API；它在同一 job 内完成：
  - 瓦片上传到 MinIO
  - 元数据写入 OpenGauss（PostgreSQL 兼容协议）
...
旧版本的 `cube_web` 曾保留一套独立的 optical ingest 接口：
...
### 6.3 optical ingest API 已退役

M3 已移除 `/v1/ingest/optical/preview` 与 `/v1/ingest/optical/confirm`，以及
前端对应的遗留前缀。当前生产 `run`、批次重试和失败资产重试负责完整的剖分与入库链路。

如果未来需要补入库工具，必须以当前严格数据集域和质量契约单独设计，不得恢复以下旧接口：
...
3. 仅在新的严格数据集域契约下评估是否需要“仅入库重试”工具。
